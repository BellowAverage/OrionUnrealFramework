...
void UOrionBPFunctionLibrary::DisablePhysicsOnActor(AActor* TargetActor)
{
	if (TargetActor)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Disabled physics and collision for all StaticMeshComponents of %s"),
		       *TargetActor->GetName());
...
void UOrionBPFunctionLibrary::DisablePhysicsAndCollision(UActorComponent* Component)
{
	...
	{
		// 其他组件类型不处理
		UE_LOG(LogTemp, Warning, TEXT("Component %s is not a StaticMeshComponent and was skipped."),
		       *Component->GetName());
	}
}
...
bool UOrionBPFunctionLibrary::CheckActorPhysicsStatus(AActor* TargetActor)
{
	...
	for (UActorComponent* Component : Components)
	{
		if (UStaticMeshComponent* StaticMeshComp = Cast<UStaticMeshComponent>(Component))
		{
			...
			{
				bAllDisabled = false;
				UE_LOG(LogTemp, Warning, TEXT("StaticMeshComponent with enabled feature: %s"),
	...
	{
		UE_LOG(LogTemp, Log,
		       TEXT("All StaticMeshComponents of %s and its children have physics, gravity, and collision disabled."),
		       *TargetActor->GetName());
	}
...
void UOrionBPFunctionLibrary::FaceComponentToPlayerCamera(USceneComponent* Component)
{
	if (!Component)
	{
		return;
	...
	const FVector CompLoc = Component->GetComponentLocation();

	// compute look-at rotation
	const FRotator LookAtRot = UKismetMathLibrary::FindLookAtRotation(CompLoc, CameraLoc);
